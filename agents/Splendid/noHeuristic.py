...
class Splendor(MDP):
    ...
    def getReward(self, state, action, nextState):  # Allison
        ...
        if 'buy' in action['type']:
            ...
            reward_point += 2*card.points #- (len(agent.cards[card.colour])**2 + cost)/(card.deck_id+1)
            for noble in state.board.nobles: # More points if development relevant to nobles
            ...
            if len(action['collected_gems'])>1:
                for colour in action['collected_gems']: # More points if get gems relevant to cards dealt
                    for row in nextState.board.dealt+[agent.cards['yellow']]:
                        for card in row:
                            if colour in card.cost and agent.gems[colour]+len(agent.cards[colour]) < card.cost[colour]: #Gems are close to cards on board
                                cost = sum(card.cost.values())+len(card.cost)
                                reward_point += (card.points+1)/(cost-agent.gems[colour]-len(agent.cards[colour]))
                                for noble in state.board.nobles: # More points if gems go to buying developments relevant to nobles
                                    if card.colour in noble[1] and len(agent.cards[card.colour])<noble[1][card.colour]:
                                        reward_point+= (card.points+2)/(cost+noble[1][card.colour]-len(agent.cards[card.colour])-card.deck_id)
            else:
