...
class myAgent(Agent):
    ...
    def SelectAction(self,actions,game_state):
        ...
        try:
            ...
            # profiler.enable()
            action = self.player.mcts(game_state, ROUND_TIME+STARTUP_TIME-ROUND_END_BUFFER).getBestAction()
            # profiler.disable()
...
class Splendor(MDP):
    ...
    def getReward(self, state, action, nextState):  # Allison
        ...
        if nextState.agents[aid].score >= 15 and nextState.agents[aid].score>=opp.score: # Winning is ultimate reward
            reward_point += 9999 #+ nextState.agents[aid].score/sum(map(len,nextState.agents[aid].cards))
        if 'buy' in action['type']:
            card = action['card']
            cost = sum(action['returned_gems'].values())
            reward_point += card.points/(cost+5) # Reward development based on point to cost ratio
                    ...
                    for row in nextState.board.dealt+[agent.cards['yellow']]:
                        for card in row:
                            if colour in card.cost and agent.gems[colour]+len(agent.cards[colour]) < card.cost[colour]: #Reward picking gems if it can be used next buy
                                ...
                                reward_point += (card.points+0.05)/cost # Rewards for gems for each development with more reward if closer to buying
    ...
    def isTerminal(self, state):
        for agent in state.agents:
            if agent.score >= 15 and state.agent_to_move==0: #Game is over after last player's action and a player is over 15 points
                return True
        return False
...
class MCTS():
    ...
    def heuristic(self, action, state):
        ...
        if agent.score+noble >= 15: return value+50
        if 'buy' in action['type']:
            card = action['card']
            if agent.score+card.points+noble>=15: return value+50
    ...
    def simulate(self, node):
        ...
        depth = 0
        while depth<SEARCH_DEPTH and not self.mdp.isTerminal(state): #NOTE: SAME AS BEFORE. STOP EARLY
            ...
            if len(agent.cards['yellow']) < 3:
                ...
                for returned_gems in return_combos:
                    for card in board.dealt_list():
                        if card and ((card.points)/(sum(card.cost.values()))>0.24 or card.points+opp.score>=15):
                            c1 = c2 = 0
                            for colour in card.cost:
                            ...
                            deck = card.deck_id
                            if c1 == 0 or (c1>1 and c2>0) or deck<1: #(opp.score>0 and agent.score>0 and deck==0) or (opp.score==0 and agent.score==0 and deck>1):
